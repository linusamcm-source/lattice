...
per-language `LanguageConfig`, emits `function` nodes for the language's function node-kinds (name via
the grammar's `name` field) as children of the file, and `variable` nodes for the language's local
bindings as children of their enclosing function — mirroring the Rust path's ids, `contains` edges,
and `meta` (function/variable nodes carry `meta.language`; the file node's `meta` stays `None`).
Error handling mirrors Phase 0/1: panic-free, and the `file` node is marked `status: error` when the
tree's root reports an error.
...
the Rust path: a `file` node, `function` nodes (`fn:<path>:<name>`, parent the file, `meta.language =
"python"`, `meta.range` from the node's start/end, plus a `contains` edge), and `variable` nodes
(`var:<path>:<fn>:<name>`, parent the function, `contains` edge) for simple local assignment targets
inside a function body. Panic-free: never `.unwrap()`/`panic!` outside `#[cfg(test)]`; if the parsed
tree's root node reports an error, the `file` node's `status` is `error`.
...
with a TypeScript `LanguageConfig` (functions: `function_declaration`, methods, and arrow functions
bound in a `variable_declarator`; locals: `lexical_declaration` / `variable_declarator` names).
`meta.language == "typescript"`. Same ids, `contains` edges, and panic-free error handling as P2-1.
...
a bare `file` node (`status: unknown`, no children). Extend the watcher: add
`pub fn is_source_file(path: &Path) -> bool` accepting `rs`/`py`/`ts` (keep `is_rust_file` or
re-point the callback + its tests to `is_source_file`). Route `app.rs` through `parse_source` (in
`ingest_file`) and replace the `run()` initial-parse `rs`-only filter with `is_source_file`. The
Rust path, its ids, and all Phase-0/1 tests remain green.
